...
def generate_samples(n, m, sample_id, num_gates, a=1., b=1.):
    num_qubits = n
...
def generate_dataframe():
    #samples = glob.glob('free_fermion_stats/*_metrics.json')
    samples = glob.glob('random_H/*_metrics.json')
    analysis = {}
...
def plot_metrics(df, axes):
    ax = df.plot(kind="scatter", x=axes[0], y=axes[1], ec='r', fc='none')
